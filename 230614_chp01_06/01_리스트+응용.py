...
print("=" * 50)
print(sorted(r_number, reverse=True)) # reversed -> 뒤집어진, sorted -> 정렬된
# sorted 정렬 -> 오름차순 -> 뒤집겠니?
# 오르막 -> 오름차순. (ascending) - asc
# 1, 2, 3, ...
...
a[0] = 100
a[1] = 150
print("b : ", b) # a가 변화함에 따라 b도 같이 변화하는 할당에 따른 복사하는 방법
print(a is b) # 이러한 문제가 생긴다. 그러면 할당 말고 이러한 문제를 피할 수 있는 방법은 ?

c  = a[:]
print(a is c) # 2개의 주소가 같지 않다
a[0] = 1200
a[1] = 100
a[2] = 50
print("c : ", c)
# 슬라이스로 복사한 경우에는 할당이 아니라 '얕은 복사'이므로 서로 영향을 미치지 않는다.
d = a.copy() # 얕은 복사.
print(c is d) # 얕은 복사 이므로 c와 d를 서로 영향 주지 않는다.
...
for v in b:
    ...
    if max_n < v:
        max_n = v
print("b 리스트의 최댓값은 : ", b, max_n)

for v in b:
    # 최솟값을 구하는 방법
    if min_n == 0 or min_n > v:
        min_n = v
print("b 리스트의 최솟값은 : ", b, min_n)
...
## -> 리스트 안에 for 반복문과 if 조건문 -> 값들의 묶음을 표현.
...
for i in range(10): # i <- 안 겹치게만.
    for j in range(10): # 시퀀스에서 값을 빼내올 때 쓰는 변수명이 안겹치게만 조심.
        print(i, j, i * j)
        # 삼중 ... 사중...
print("=" * 30)
# 이중 for 문 ? 이중 리스트 컴프리헨션
